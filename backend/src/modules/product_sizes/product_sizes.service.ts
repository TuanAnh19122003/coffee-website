...
export class ProductSizesService {
  ...
  async findAll(page: number, limit: number) {
    const [product_sizes, totalItems] =
      await this.productSizesRepository.findAndCount({
        skip: (page - 1) * limit,
        take: limit,
        relations: ['product'],
      });
  ...
  async create(
    ...
    const product_size =
      this.productSizesRepository.create(createProductSizeDto);
    if (createProductSizeDto.productId) {
      product_size.product = await this.productsService.findOne(
  ...
  async update(
    ...
    if (updateProductSizeDto.productId) {
      product_size.product = await this.productsService.findOne(
        updateProductSizeDto.productId,
      );
    }
